...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($topicArray)) {
        ...
        foreach ($topicArray as $topicId) {
            if (!is_numeric($topicId)) continue;
            $insert->execute([$id, (int) $topicId]);
        }
    }

    /* SYNC RELATED LISTINGS */
    $deleteUrl = $projectUrl . "/rest/v1/RelatedListing?listingID=eq." . urlencode($id) . "&type=eq.listing";
    ...
    if (!empty($_POST['relatedListingIDs'])) {
        foreach ($_POST['relatedListingIDs'] as $relatedID) {
            ...
            if (!empty($relatedID)) {
                $titleLookupUrl = $projectUrl . "/rest/v1/Listing?select=title&listingID=eq." . urlencode($relatedID) . "&limit=1";
                $chTitle = curl_init($titleLookupUrl);
                curl_setopt_array($chTitle, [
                    CURLOPT_HTTPHEADER => ["apikey: $key", "Authorization: Bearer $key"],
                    CURLOPT_RETURNTRANSFER => true
                ]);
                $titleRes = json_decode(curl_exec($chTitle), true);
                curl_close($chTitle);
                $relatedTitle = $titleRes[0]['title'] ?? "Related Listing #" . $relatedID;
...
<script>
    ...
    window.removeRelated = function(btn) {
